...
//First main part (Solution construction phase)
...
//printf("The solution len is %f \n", antColony[i].solutionLen);
}
//Second main part (Pheromone update process)
int from, to;
...
__global__ void constructSolution(struct ant *antColony_d, curandState *state_d, float *heuristic_d, double *phero_d, int THREADS){
 ...
 //printf("Retrieving values in device: %5.5f\n", phero_d[1889*1000]);
 int ant_id = THREADS * blockIdx.x + threadIdx.x;
 int node = 0;
 
 while (node++ < NODES){
  //printf("The value of node is : %d", node);
  ...
  if (antColony_d[ant_id].pathIndex < NODES){
   //printf("we are inside lets calculate next node\n");
   //printf("path index: %d\n", antColony_d[ant_id].pathIndex);
   ...
   //Put the nde in the tabu list and in the solution list
...
__device__ double probFunctionProduct(int from, int to, double *phero_d, float *heuristic_d){
  double r; 
  r = pow( phero_d[from + to*NODES], ALPHA) * pow(1/(heuristic_d[from + to*NODES]), BETA);
  //r = phero_d[1836545];
  //printf("r value is : %5.5f\n", r);
  if (!isnan(r)){
  return (double) (( r ));
 }
 else{
...
__device__ int NextNode(struct ant *antColony_d, int pos, float *heuristic_d, double *phero_d, curandState *state_d){
 ...
 for(to = 0; to < NODES; to++){
  ...
  //printf("denom is %5.8f\n", denom);
  }
 //printf("from: %d ", antColony_d[pos].curNode);
 //printf("to: %d \n", to);
 }
 //printf("denom is %5.8f", denom);
 assert(denom != 0.0);
 //printf("we pass assert");
 to++;
